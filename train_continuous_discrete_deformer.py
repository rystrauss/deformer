...
def load_datasets(dataset, batch_size):
    train_ds, ds_info = tfds.load(dataset, split="train", with_info=True)
    val_ds = tfds.load(dataset, split="validation")

    classes_per_feature = ds_info.metadata["classes_per_feature"]
    ...
    return train_ds, val_ds, classes_per_feature
...
@click.option(
    ...
    type=click.INT,
    default=32,
    help="The dimensionality of the class embeddings.",
)
@click.option(
    "--mixture_components",
    type=click.INT,
...
def main(
    ...
    run = wandb.init(
        project="deformer",
        job_type="train_continuous_discrete_deformer",
        mode="disabled" if offline else "online",
        config=config,
    )

    train_ds, val_ds, classes_per_feature = load_datasets(dataset, batch_size)

    noise_mask = np.expand_dims(np.asarray(classes_per_feature) < 2, axis=0)
    ...
    def loss_fn(step, is_training, x):
        ...
        if data_noise != 0:
            x += (
                ...
                * data_noise
                * noise_mask  # Only add noise to continuous features
                * is_training
            )

        lls = model(x, order, is_training)
        lls = jnp.sum(lls, axis=-1)
    ...
    state_artifact = wandb.Artifact(
        f"{dataset}_continuous_discrete_deformer",
        type="continuous_discrete_deformer_model",
    )
